...
class Plot:
    ...
    def __init__(
        ...
        if self.sim:
            ...
            self.obs_fig, self.obs_ax = plt.subplots(figsize=sim_figsize)
            # remove axes ticks and labels to speed up animation
            self.obs_ax.set_xlabel('')
            self.obs_ax.set_xticks([])
            self.obs_ax.set_ylabel('')
            self.obs_ax.set_yticks([])
            self.ax.append(self.obs_ax)
            ...
            self.obs_ax.plot(ship_pos[0], ship_pos[1], 'b-', label='ship path')
            ...
            if ship_vertices is not None:
                ...
                self.obs_ship_patch = self.obs_ax.add_patch(
                    patches.Polygon(ship_vertices @ R(ship_pos[2]).T + ship_pos[:2], True, fill=True,
                                    edgecolor='black', facecolor='white', linewidth=2)
                )
    ...
    def update_ship(self, body, shape, move_yaxis_threshold=20) -> None:
        ...
        if (
            ...
            self.obs_ax.set_ylim([ymin + offset[1], ymax + offset[1]])
    ...
    def update_obstacles(self, polygons: List = None, obstacles: List = None) -> None:
        if polygons:
            ...
            for obs, patch in zip(obstacles, self.sim_obs_patches):
                patch.set_xy(obs)
            for obs, patch in zip(obstacles, self.obs_patches):
                patch.set_xy(obs)
    ...
    def animate_obs(self, save_fig_dir=None, suffix=0):

        self.obs_fig.canvas.blit(self.obs_fig.bbox)
        self.obs_fig.canvas.flush_events()
        self.save_obs(save_fig_dir, suffix, fig='sim')

    def save_obs(self, save_fig_dir, suffix, im_format='png', fig='map'):
        if save_fig_dir:
            if not os.path.isdir(save_fig_dir):
                os.makedirs(save_fig_dir)
            fp = os.path.join(save_fig_dir, str(suffix) + '_obs.' + im_format)  # pdf is useful in inkscape
            if fig == 'map':
                self.map_fig.savefig(fp)
            else:
                plt.axis('off')
                self.obs_fig.savefig(fp, bbox_inches="tight", transparent=False)
            return fp
